...
import { 
  TrendingDown, Plus, Calendar, Tag, X, Receipt
} from 'lucide-react';
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1600px] mx-auto pb-20">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-card-foreground">Expenses</h1>
          <p className="text-muted-foreground">Track farm costs and spending</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-card p-6 rounded-xl shadow-sm border border-border flex items-center gap-4">
            ...
            <div><p className="text-sm text-muted-foreground font-medium">Total Expenses</p><h3 className="text-2xl font-bold text-card-foreground">GH₵ {totalExpense.toLocaleString()}</h3></div>
        </div>
        <div className="bg-card p-6 rounded-xl shadow-sm border border-border flex items-center gap-4">
            <div className="p-3 rounded-lg bg-orange-50 text-orange-600"><Calendar className="w-6 h-6" /></div>
            <div><p className="text-sm text-muted-foreground font-medium">This Month</p><h3 className="text-2xl font-bold text-card-foreground">GH₵ {thisMonth.toLocaleString()}</h3></div>
        </div>
      </div>

      {/* Transactions List */}
      <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
        <div className="p-4 border-b border-border font-medium text-foreground bg-muted/50">Recent Transactions</div>
        <div className="divide-y divide-border">
            {expenses.length === 0 && <div className="p-8 text-center text-muted-foreground">No expenses recorded yet.</div>}
            {expenses.map((exp: any) => (
                <div key={exp.id} className="p-4 flex items-center justify-between hover:bg-muted transition-colors">
                    <div className="flex items-center gap-4">
                        <div className="p-2 bg-secondary rounded-full text-muted-foreground"><Receipt className="w-5 h-5" /></div>
                        <div>
                            <p className="font-bold text-card-foreground">{exp.title}</p>
      ...
      {/* Add Modal */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-overlay flex items-center justify-center z-50 p-4 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}>
            <div className="bg-card text-card-foreground border border-border rounded-xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-bold">Record Expense</h2>
                    <button onClick={() => setIsModalOpen(false)}><X className="text-muted-foreground" /></button>
                </div>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-xs font-bold text-muted-foreground uppercase">Amount (GH₵)</label>
                    ...
                    <div>
                        ...
                        <select name="category" className="w-full border p-3 rounded-lg bg-card outline-none focus:border-red-500 mt-1">
                            <option>Seeds</option>
