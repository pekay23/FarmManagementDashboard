...
import { 
  Sprout, MapPin, Calendar, BarChart3, Plus, 
...
export default function CropManagement() {
  ...
  async function handleSaveCrop(e: any) {
    e.preventDefault();
    const initialStatus = 'Growing' as const; 
    const formData = {
  ...
  async function handleSaveYield(e: any) {
    e.preventDefault();
    if (!selectedCrop) return;
    const newStatus = 'Harvested' as const;
    try {
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1600px] mx-auto min-h-screen pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-card-foreground">Crop Management</h1>
          <p className="text-muted-foreground">Manage your farm plots and crop activities</p>
        </div>
        <button 
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        <div className="flex-1 w-full space-y-4">
          {!crops && <div className="text-center py-10 text-muted-foreground"><Loader2 className="w-6 h-6 animate-spin mx-auto"/></div>}
          {crops && crops.length === 0 && (
            <div className="bg-muted border-2 border-dashed border-border rounded-xl p-10 text-center">
          ...
          {crops?.map((crop: any) => {
            ...
            <div 
              ...
              className={`bg-card p-5 rounded-xl border-2 cursor-pointer transition-all hover:shadow-md 
                ${selectedCrop?.id === crop.id ? 'border-primary-500 ring-1 ring-primary-500 shadow-md' : 'border-border'}`}
            >
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h3 className="text-lg font-bold text-card-foreground flex items-center gap-2">
                    {crop.plot_number} <span className="text-muted-foreground font-normal">|</span> {crop.crop_type}
                  </h3>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-xs text-muted-foreground">{crop.location}</span>
                  </div>
                </div>
                <StatusBadge status={status} />
              </div>
              <div className="grid grid-cols-2 gap-4 text-xs mt-4 pt-3 border-t border-border">
        ...
        {selectedCrop ? (
          <div className="w-full lg:w-96 bg-card rounded-xl shadow-lg border border-border p-6 sticky top-4 animate-in slide-in-from-right-4 duration-300">
            <div className="mb-6 pb-6 border-b border-border flex justify-between items-start">
              <div>
                <h2 className="text-xl font-bold text-card-foreground">{selectedCrop.plot_number} Details</h2>
                <p className="text-muted-foreground font-medium">{selectedCrop.crop_type} - {selectedCrop.variety}</p>
              </div>
              <div className="flex items-center gap-1">
                  <button onClick={() => openEditCrop(selectedCrop)} className="p-2 hover:bg-secondary rounded-full text-muted-foreground hover:text-primary-600 transition-colors">
                      <Pencil className="w-4 h-4" />
                  </button>
                  <button onClick={() => setConfirmDelete(selectedCrop)} className="p-2 hover:bg-red-50 rounded-full text-muted-foreground hover:text-red-600 transition-colors">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                  <h4 className="font-semibold text-card-foreground text-xs uppercase tracking-wide opacity-80">Recent Treatments</h4>
                  {treatments?.length === 0 && <p className="text-xs text-muted-foreground italic">No treatments recorded.</p>}
            ...
            <div className="hidden lg:flex w-96 bg-muted rounded-xl border-2 border-dashed border-border p-10 flex-col items-center justify-center text-muted-foreground h-64 sticky top-4">
      ...
      {confirmDelete && (
           <div className="fixed inset-0 bg-overlay flex items-center justify-center z-50 p-4 backdrop-blur-sm">
             <div className="bg-card text-card-foreground border border-border rounded-xl w-full max-w-sm p-6 shadow-2xl text-center animate-in zoom-in-95">
                 ...
                 <h3 className="text-lg font-bold text-card-foreground mb-2">Delete Crop?</h3>
                 <p className="text-muted-foreground text-sm mb-6">Are you sure you want to delete crop <strong>{confirmDelete.plot_number}</strong>? This will also delete all of its treatment history. This action cannot be undone.</p>
                 <div className="flex gap-3">
                     <button onClick={() => setConfirmDelete(null)} className="flex-1 py-2.5 bg-secondary rounded-lg font-bold text-muted-foreground hover:bg-secondary/60">Cancel</button>
      ...
      {isCropModalOpen && (<Modal title={editingCrop ? "Edit Crop Details" : "Add New Crop"} onClose={() => setIsCropModalOpen(false)}> <form onSubmit={handleSaveCrop} className="space-y-4"><input name="plot_number" required defaultValue={editingCrop?.plot_number} type="text" placeholder="Plot Number (e.g. A001)" className="w-full border p-3 rounded-lg outline-none focus:border-primary-500" /><input name="crop_type" required defaultValue={editingCrop?.crop_type} type="text" placeholder="Crop Type (e.g. Maize)" className="w-full border p-3 rounded-lg outline-none focus:border-primary-500" /><div className="grid grid-cols-2 gap-4"><input name="variety" defaultValue={editingCrop?.variety} type="text" placeholder="Variety" className="w-full border p-3 rounded-lg outline-none focus:border-primary-500" /><input name="plot_size_acres" required defaultValue={editingCrop?.plot_size_acres} type="number" step="0.1" placeholder="Size (acres)" className="w-full border p-3 rounded-lg outline-none focus:border-primary-500" /></div><div className="grid grid-cols-2 gap-4"><input name="location" defaultValue={editingCrop?.location} type="text" placeholder="Location" className="w-full border p-3 rounded-lg outline-none focus:border-primary-500" /><input name="estimated_yield_kg" defaultValue={editingCrop?.estimated_yield_kg} type="number" placeholder="Est. Yield (kg)" className="w-full border p-3 rounded-lg outline-none focus:border-primary-500" /></div><div className="grid grid-cols-2 gap-4"><div className="space-y-1"><label className="text-xs text-muted-foreground font-bold ml-1 uppercase">Planting Date</label><input name="planting_date" required defaultValue={editingCrop ? new Date(editingCrop.planting_date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]} type="date" className="w-full border p-3 rounded-lg text-muted-foreground outline-none focus:border-primary-500" /></div><div className="space-y-1"><label className="text-xs text-muted-foreground font-bold ml-1 uppercase">Exp. Harvest</label><input name="expected_harvest_date" defaultValue={editingCrop?.expected_harvest_date ? new Date(editingCrop.expected_harvest_date).toISOString().split('T')[0] : ''} type="date" className="w-full border p-3 rounded-lg text-muted-foreground outline-none focus:border-primary-500" /></div></div><button type="submit" className="w-full bg-primary-600 hover:bg-primary-700 text-white py-3 rounded-lg font-bold mt-4">{editingCrop ? "Update Crop" : "Save Crop"}</button></form> </Modal>)}
      {isYieldModalOpen && selectedCrop && (<Modal title="Update Yield" onClose={() => setIsYieldModalOpen(false)}> <form onSubmit={handleSaveYield} className="space-y-4"><div className="bg-muted p-3 rounded-lg mb-4"><p className="text-sm text-muted-foreground">Estimated Yield</p><p className="text-xl font-bold text-card-foreground">{selectedCrop.estimated_yield_kg} kg</p></div><div className="space-y-1"><label className="text-sm font-bold text-foreground">Actual Yield (kg)</label><input name="actual_yield" type="number" required defaultValue={selectedCrop.actual_yield_kg} placeholder="Enter actual yield" className="w-full border p-3 rounded-lg outline-none focus:border-primary-500" /></div><div className="space-y-1"><textarea name="harvest_notes" rows={3} defaultValue={selectedCrop.harvest_notes} placeholder="Harvest notes (optional)" className="w-full border p-3 rounded-lg resize-none outline-none focus:border-primary-500" /></div><div className="flex gap-4 mt-4"><button type="button" onClick={() => setIsYieldModalOpen(false)} className="flex-1 bg-secondary hover:bg-secondary/60 text-foreground py-3 rounded-lg font-medium">Cancel</button><button type="submit" className="flex-1 bg-yellow-500 hover:bg-yellow-600 text-white py-3 rounded-lg font-bold">Update Yield</button></div></form> </Modal>)}
      {isTreatmentModalOpen && (<Modal title="Add Treatment" onClose={() => setIsTreatmentModalOpen(false)}> <form onSubmit={handleSaveTreatment} className="space-y-4"><select name="treatment_type" className="w-full border p-3 rounded-lg bg-card outline-none focus:border-primary-500"><option>Fertilizer</option><option>Pesticide</option><option>Irrigation</option><option>Pruning</option><option>Other</option></select><input name="product_name" type="text" placeholder="Product Name (e.g. NPK 15-15-15)" className="w-full border p-3 rounded-lg outline-none focus:border-primary-500" /><div className="space-y-1"><label className="text-xs text-muted-foreground font-bold ml-1 uppercase">Date</label><input name="treatment_date" required type="date" defaultValue={new Date().toISOString().split('T')[0]} className="w-full border p-3 rounded-lg text-muted-foreground outline-none focus:border-primary-500" /></div><div className="grid grid-cols-2 gap-4"><input name="quantity" type="text" placeholder="Quantity (e.g. 50kg)" className="w-full border p-3 rounded-lg outline-none focus:border-primary-500" /><div className="relative"><DollarSign className="w-4 h-4 absolute left-3 top-3.5 text-muted-foreground" /><input name="cost" type="number" step="0.01" placeholder="Cost" className="w-full border pl-9 p-3 rounded-lg outline-none focus:border-primary-500" /></div></div><textarea name="notes" placeholder="Notes (optional)" rows={3} className="w-full border p-3 rounded-lg resize-none outline-none focus:border-primary-500" /><button type="submit" className="w-full bg-primary-600 hover:bg-primary-700 text-white py-3 rounded-lg font-bold">Save Treatment</button></form> </Modal>)}
    </div>
  );
}
